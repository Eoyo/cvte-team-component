...
function BuildBigReducer(
  ...
  const normal = (
    ...
    const isSpecialRus = specialActions(
      state,
      ac as any
    );
    ...
    if (isSpecialRus) {
      ...
      typeof reducers[ac.type] === "function"
    ) {
      ...
      if (
        // is generator
        typeof reducerRus[Symbol.iterator] ===
        "function"
      ) {
        ...
        next(() => {
          Senior(reducerRus, (yetRus: any) => {
            if (typeof yetRus === "function") {
              ...
              !Array.isArray(yetRus) &&
              typeof yetRus === "object"
            ) {
              ...
              console.log(
                "Actor reducer wrong state return @" +
                  `*[${ac.type}]`,
                yetRus
              );
              return;
...
type oneReducer<S, A, x extends keyof A> = (
  ...
  | mergeObj<S>
  | IterableIterator<
      mergeObj<S> | (() => void) | Promise<any>
    >;
type ActorReducer<S, A> = {
  [x in keyof A]: oneReducer<S, A, x>
};

export function Actor<S>(state: S) {
  return function Actions<A extends object>(
    actions: A
  ) {
    return function Reducers(
      ...
      // 建立 reducer , store
      const isAlways = reducers.always
        ? "always"
        : "normal";
      const bigReducer = BuildBigReducer(
        reducers,
        actions,
        isAlways
      );
      ...
      Object.getOwnPropertyNames(actions).forEach(
        d => {
          // @ts-ignore; 注入
          rus[d] = (prop: any) => {
            store.dispatch({
